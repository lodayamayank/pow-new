...
function Services() {
  ...
  const { SERVICES_PATH, SERVICES_CREATE_PATH } = useRoutePaths()
            ...
            <div className="p-1 min-w-full inline-block align-middle">
              <div className="bg-white shadow border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-slate-900 dark:border-gray-700">
                <div className="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-gray-700">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      <Breadcrumbs links={[{title: 'Services', path: SERVICES_PATH, isActive: true}]} />
                    </p>
                  </div>
                  ...
                  <div>
                    <div className="inline-flex gap-x-2">
                      ...
                      <CanAccess permissions={['add_services']}>
                        <a href={SERVICES_CREATE_PATH} className="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                ...
                {!isLoading && records.length == 0 && (
                  <NoRecordsFoundMessage actionUrl={SERVICES_CREATE_PATH} actionLabel='Create new record' heading='No records found' subHeading='create a new record to get started' />
                )}
                    ...
                    {!isLoading && records?.length > 0 ? (
                      ...
                      <tr className="bg-white hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-800">
                        ...
                        <td className="h-px w-px">
                          <div className="px-6 py-2 truncate w-64">
                            {record.description}
                          </div>
                        </td>
                        <td className="h-px w-px whitespace-nowrap">
                          ...
                          <Menu as="div" className="relative inline-block text-left">
                            ...
                            >
                              <Menu.Items className="absolute right-0 z-10 mt-2 w-36 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                <div className="py-1">
                                  <CanAccess permissions={['update_services']}>
                                    <Menu.Item>
                                      {({ active }) => (
                                        <a
                                          href={`/services/update/${record.uuid}`}
                                  ...
                                  <CanAccess permissions={['delete_services']}>
                                  <Menu.Item>
                                    {({ active }) => (
                                      <a
