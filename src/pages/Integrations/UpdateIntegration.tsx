...
import { Integration, detailsInitValues as initialValues } from './core/_models'
import { updateRecord, getRecordById } from './core/_requests'
import { CanAccess, Breadcrumbs, Loader, AlertMessage } from '@/components'
...
function UpdateIntegration() {
    const { id } = useParams()
    const [data, setData] = useState<Integration>(initialValues)
    const [isLoading, setIsLoading] = useState<boolean>(false)
    const { SERVICES_UPDATE_PATH, INTEGRATIONS_PATH } = useRoutePaths()
    ...
    const formik = useFormik<Integration>({
        initialValues,
        ...
        async function loadRecord(uuid: string) {
          try {
            ...
            if(response){
                ...
                formik.setFieldValue('svg_icon', response.svg_icon)
                formik.setFieldValue('is_active', response.is_active)
                formik.setFieldValue('default_active', response.default_active)
                setData(response)
            }
            setIsLoading(false)
          } catch (error) {
                                    ...
                                    <div className="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-gray-700">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                                <Breadcrumbs links={[{title: 'Integrations', path: INTEGRATIONS_PATH, isActive: false},{ title: 'Update Service ('+data.name+')', path: '', isActive: true}]} />
                                            </p>
                                        </div>
