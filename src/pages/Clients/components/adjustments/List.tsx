...
import { ClientAdjustments, ClientProfitSettled } from "../../core/_models";
import { PaginationState, pagination_items_per_page } from "@/utils";
import { deleteClientAdjustmentsRecord, getClientAdjustmentsRecords} from "../../core/_requests";
...
export function List(props: Props) {
    ...
    const [records, setRecords] = useState<ClientAdjustments[]>([])
    ...
    async function loadRecords() {
        try {
            setIsLoading(true)
            const response = await getClientAdjustmentsRecords(clientId,stringifyRequestQuery({search,page,items_per_page}))
    ...
    const deleteRecordLocal = async (uuid:string) => {
        try{
            setIsLoading(true)
            const resp = await deleteClientAdjustmentsRecord(clientId,uuid)
                ...
                <tbody>
                    {records && records.map((record) => (
                        <tr key={record.uuid}>
                            ...
                            <td>{record.amount}</td>
                            <td>{moment(record.entryDate).format("DD/MM/YYYY")}</td>
                            <td className="text-end text-nowrap">
                                <CanAccess permissions={['update_clients_adjustments']}>
                                    <button type='button'
                                ...
                                <CanAccess permissions={['delete_clients_adjustments']}>
                                    <button type='button'
