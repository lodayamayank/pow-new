...
import { ClientAdjustments, clientAdjustmentsDetailsInitValues as initialValues } from "../../core/_models";
import { getClientAUMRecordById, getClientAdjustmentsRecordById, updateClientAdjustmentsRecord } from "../../core/_requests";
...
export function Create(props: Props) {
    ...
    async function loadRecord(uuid: string) {
        try {
          setIsLoading(true)
          const response = await getClientAdjustmentsRecordById(clientId, uuid)
          if(response){
                formik.setFieldValue('amount', response.amount)
                formik.setFieldValue('entryDate', moment(response.entryDate).format("YYYY-MM-DD"))
                setData(response)
          }
          setIsLoading(false)
        } catch (error) {
    ...
    const formik = useFormik<ClientAdjustments>({
        initialValues,
        ...
        try {
            ...
            const respData = await updateClientAdjustmentsRecord(clientId,currentRecordId,values)
            setIsLoading(false)
