...
import { getClientGroupRecordById, updateClientGroupRecord } from '../../core/_requests'
...
function Update() {
    ...
    const formik = useFormik<ClientGroup>({
        ...
        try {
            ...
            const respData = await updateClientGroupRecord(formData, data && data.uuid ? data.uuid : id)
            setIsLoading(false)
    ...
    useEffect(() => {
        async function loadRecord(uuid: string) {
          try {
            setIsLoading(true)
            const response = await getClientGroupRecordById(uuid)
            if(response){
    ...
    const breadCrumbs = [
        {title: 'Client Groups', path: CLIENT_GROUPS_PATH, isActive: true},
        {title: `Update Client Group Details: ${data ? data.name : ''}`, path: '', isActive: true}
    ]
