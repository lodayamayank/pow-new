...
import { ClientGroup } from '../../core/_models'
import { deleteClientGroupRecord, getClientGroupRecords } from '../../core/_requests'
...
function List() {
  ...
  const { CLIENT_GROUPS_PATH, CLIENT_GROUPS_CREATE_PATH } = useRoutePaths()
  const navigate = useNavigate()
  async function loadUsers() {
    try {
      setIsLoading(true)
      const response = await getClientGroupRecords(stringifyRequestQuery({search,page,items_per_page}))
  ...
  const deleteRecordLocal = async (uuid:string) => {
    try{
      setIsLoading(true)
      const resp = await deleteClientGroupRecord(uuid)
      setIsLoading(false)
      loadUsers()
    } catch (error) {
      const err = error as AxiosError
                    ...
                    <div className="col-12">
                      <div className="card">
                        <div className="card-header">
                          <h3 className="card-title">Client Groups</h3>
                          <div className="card-actions">
                              <CanAccess permissions={['add_masterdata_brokers']}>
                                <button 
                                type='button' 
                                onClick={() => navigate(CLIENT_GROUPS_CREATE_PATH)}
                        ...
                        <div className="table-responsive">
                          {records.length == 0 ? (
                            <NoRecordsFoundMessage actionUrl={CLIENT_GROUPS_CREATE_PATH} actionLabel='Create new group' heading='No groups found' subHeading='create a new group to get started' />
                          ):(
