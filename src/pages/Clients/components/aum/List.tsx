...
import { ClientAUM, ClientBroker } from "../../core/_models";
import { PaginationState, pagination_items_per_page } from "@/utils";
import { deleteClientAUMRecord, deleteClientBrokerRecord, getClientAUMRecords, getClientBrokerRecords } from "../../core/_requests";
...
export function List(props: Props) {
    ...
    const [records, setRecords] = useState<ClientAUM[]>([])
    ...
    async function loadRecords() {
        try {
            setIsLoading(true)
            const response = await getClientAUMRecords(clientId,stringifyRequestQuery({search,page,items_per_page}))
    ...
    const deleteRecordLocal = async (uuid:string) => {
        try{
            setIsLoading(true)
            const resp = await deleteClientAUMRecord(clientId,uuid)
                ...
                <tbody>
                    {records && records.map((record) => (
                        <tr key={record.uuid}>
                            ...
                            <td>{record.amount}</td>
                            <td>{moment(record.startDate).format("DD/MM/YYYY")}</td>
                            {/* <td>{record.endDate ? moment(record.endDate).format("DD/MM/YYYY") : ''}</td> */}
                            <td className="text-end text-nowrap">
                                <CanAccess permissions={['update_clients_aum']}>
                                    <button type='button'
                                ...
                                <CanAccess permissions={['delete_clients_aum']}>
                                    <button type='button'
