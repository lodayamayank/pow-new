...
import {
  ...
  getRecords as getClientsRecords
} from '@/pages/Clients/core/_requests'
import { AxiosError } from 'axios'
import React, { useEffect, useRef, useState } from 'react'
import moment, { Moment } from 'moment'
import { getAllTradeData } from '../core/_requests'
import {
  calcTotalETFGainsLiquidDB,
  calcTotalETFGainsOthersDB,
  calculatePercentage,
  sumOfValuesForKey
} from '@/utils'
...
export default function Landing(props: Props) {
  ...
  const [selectedStartDate, setSelectedStartDate] = useState<string>(
    moment('2024-04-01', 'YYYY-MM-DD').format('YYYY-MM-DD')
  )
  ...
  function calculateTotalWeeks(startDate, endDate) {
    ...
    const totalWeeks = Math.ceil(
      differenceMilliseconds / (1000 * 60 * 60 * 24 * 7)
    )
    return totalWeeks
  }
  ...
  function getWeekDateRanges(startDate, endDate) {
    ...
    let financialYearEnd = moment(financialYearStart)
      .add(1, 'year')
      .subtract(1, 'day') // Financial year ends on 31st March next year
    ...
    while (currentWeekEnd.isSameOrBefore(endDate, 'day')) {
      if (
        currentWeekStart.isSameOrBefore(financialYearEnd) &&
        currentWeekEnd.isSameOrAfter(financialYearEnd)
      ) {
        weekDateRanges.push({
          key: `${currentWeekStart.format(
            'YYYY-MM-DD'
          )}_to_${financialYearEnd.format('YYYY-MM-DD')}`,
          start: currentWeekStart,
          end: financialYearEnd,
          formattedStart: currentWeekStart.format('YYYY-MM-DD'),
          formattedEnd: financialYearEnd.format('YYYY-MM-DD')
        })
      ...
      if (currentWeekStart.month() == 2 && currentWeekEnd.month() == 3) {
        ...
        weekDateRanges.push({
          key: `${currentWeekEnd
            .clone()
            .startOf('month')
            .format('YYYY-MM-DD')}_to_${currentWeekEnd.format('YYYY-MM-DD')}`,
          start: currentWeekEnd.clone().startOf('month'),
          end: currentWeekEnd,
          formattedStart: currentWeekEnd
            .clone()
            .startOf('month')
            .format('YYYY-MM-DD'),
          formattedEnd: currentWeekEnd.format('YYYY-MM-DD')
        })
      } else {
        weekDateRanges.push({
          key: `${currentWeekStart.format(
            'YYYY-MM-DD'
          )}_to_${currentWeekEnd.format('YYYY-MM-DD')}`,
          start: currentWeekStart,
          end: currentWeekEnd,
          formattedStart: currentWeekStart.format('YYYY-MM-DD'),
          formattedEnd: currentWeekEnd.format('YYYY-MM-DD')
      ...
      currentWeekStart = moment(currentWeekEnd)
        .add(7 - weekEnd, 'day')
        .startOf('week')
        .day(weekStart) // Move to the next Sunday
      currentWeekEnd = moment(currentWeekStart).endOf('week').day(weekEnd) // End the week on Saturday
    }
    return weekDateRanges
  }

  function getWeekDateRanges_Old(
    startDate: string,
    endDate: string
  ): {
    start: moment.Moment
    ...
    while (currentStart.isBefore(finalEnd)) {
      ...
      if (
        currentStart.isSameOrBefore(financialYearEnd) &&
      ...
      const week: Week = {
        ...
        key: `${currentStart.format(DATE_FORMAT)}_to_${weekEnd.format(
          DATE_FORMAT
        )}`,
  ...
  async function loadClients() {
    try {
      ...
      if (recordsList.length > 0) {
        const clientListForDropDown = recordsList.map(({ uuid, name }) => ({
          uuid,
          name
        }))
  ...
  async function loadTradeData() {
    ...
    // }
    let lastFiveWeeksDateRanges = getWeekDateRanges(
      selectedStartDate,
      selectedDate
    )
    ...
    await Promise.all(
      clientId.map(async (clientIdObj) => {
        const clientDetails = clients.find(
          (client) => client.uuid == clientIdObj.value
        )
        const clientBrokersResponse = await getClientBrokerRecords(
          clientIdObj.value
        )
        ...
        await Promise.all(
          clientBrokers.map(async (clientBroker) => {
            ...
            const responseProfits = await getClientProfitsRecords(
              clientIdObj.value,
              stringifyRequestQuery(qry)
            )
            if (responseProfits.data && responseProfits.data.length > 0) {
              const profitsData = responseProfits.data
              //@ts-ignore
              profitsData.sort(
                (a, b) => new Date(a.settledDate) - new Date(b.settledDate)
              )
              const latestProfitsSettledDate = moment(
                profitsData[profitsData.length - 1]['settledDate']
              )
                .utcOffset('+5:30')
                .format('YYYY-MM-DD')
              const cloestSatDateNew = closestPreviousSaturday(selectedDate)
              const totalWeeks = calculateTotalWeeks(
                new Date(getNextMondayFromDate(latestProfitsSettledDate)),
                new Date(moment(cloestSatDateNew).format('YYYY-MM-DD'))
              )
              // lastFiveWeeksDateRanges = getWeekDateRanges(cloestSatDateNew,totalWeeks)
              if (
                moment(latestProfitsSettledDate).isSameOrBefore(
                  moment(cloestSatDateNew)
                ) &&
                moment(latestProfitsSettledDate).isBefore(
            ...
            try {
              ...
              if (allTradeDataResponse.status == 201) {
                ...
                allTradeDataCp[clientBroker.clientCode] = {
    ...
    if (clientCode in allTradeData) {
      ...
      if (allTradeDataKeys.length > 0) {
        try {
          ...
          const weeklyReportTable = areaRef.current.querySelector(
            '.weekly-report-' + clientCode
          )
            ...
            Object.keys(weekData).forEach((key) => {
              if (
                ...
                if (data.length > 0) {
                  ...
                  XLSX.utils.sheet_add_aoa(ws, [[key]], {
                    origin: { r: currentRow - 1, c: 0 }
                  })
                  currentRow++ // Move to the next row
  ...
  const exportAllClientsToExcel = () => {
    console.log('Exporting all clients data')
    setIsDownloadingReport(true)
    try {
      const wb = XLSX.utils.book_new()
      let sheetIndex = 1

      // Create a report sheet for each client
      Object.keys(allTradeData)
        .sort()
        .forEach((clientCode) => {
          if (clientCode in allTradeData) {
            const allTradeDataDateKeyTradeData =
              allTradeData[clientCode]['tradeData']
            const allTradeDataKeys = Object.keys(allTradeDataDateKeyTradeData)

            // Create report sheet for this client
            const weeklyReportTable = areaRef.current.querySelector(
              '.weekly-report-' + clientCode
            )
            if (weeklyReportTable) {
              ...
              XLSX.utils.book_append_sheet(
                wb,
                ws,
                `${clientCode}_Report`.substring(0, 31)
              )
            }

            // Create data sheets for each week
            allTradeDataKeys.forEach((dateKeys) => {
              ...
              XLSX.utils.book_append_sheet(
                wb,
                ws,
                `${clientCode}_${dateKeySplit[1]}`.substring(0, 31)
              )
            })
          }
        })

      XLSX.writeFile(wb, `Weekly Report All Clients ${selectedDate}.xlsx`)
    } catch (E) {
      console.log('download error', E)
      setIsDownloadingReport(false)
    }
                          ...
                          <div className="card-header">
                            ...
                            <div className="card-actions">
                              ...
                              <button
                                ...
                                disabled={
                                  isDownloadingReport ||
                                  Object.keys(allTradeData).length === 0
                                }
                                    ...
                                    Object.keys(allTradeData)
                                      ...
                                      .map((clientCode) => (
                                        <li
                                          ...
                                          role="presentation"
                                          onClick={() =>
                                            setCurrentClientCodeTab(clientCode)
                                          }
                                        >
                                          <span
                                            className={`nav-link ${
                                              currentClientCodeTab == clientCode
                                                ? 'active'
                                                : ''
                                            }`}
                                            role="tab"
                                    ...
                                    >
                                      ...
                                      >
                                        <div className="table-responsive">
                                          <table
                                            className={`weekly-report-${clientCode} table table-vcenter table-bordered table-nowrap card-table`}
                                          >
                                            <thead>
                                              <tr>
                                                <td className="w-50">
                                                  <h2 className="p-0 m-o">
                                                    {clientCode in allTradeData
                                                      ? allTradeData[
                                                          clientCode
                                                        ]['clientDetails'][
                                                          'name'
                                                      ...
                                                      : ''}
                                                    <div className="text-secondary text-wrap fs-5">
                                                  ...
                                                  {isGenerateReport && (
                                                    <a
                                                      target="_blank"
                                                      href={`${process.env.DO_SPACES_BUCKET_CDN_URL}reports/weekly/${clientCode}/${selectedDate}.html`}
                                                  ...
                                                  <a
                                                    ...
                                                    className="btn-link me-2 cursor-pointer"
                                                  >
                                                    Download Data
                                                  </a>
                                                  {/* <button className="btn" disabled={isDownloadingReport} onClick={exportTablesToExcel}>{isDownloadingReport ? 'Downloading...' : 'Download Report'}</button> */}
                                                ...
                                                {weeks.map((week, indx) => {
                                                  ...
                                                  return (
                                                    <td className="text-center">
                                                      ...
                                                      <div className="display-9 fw-bold my-3">
                                                        {week.formattedStart}{' '}
                                                        <br />
                                                        to
                                                        <br />{' '}
                                                  ...
                                                  weeks.map((week, indx) => {
                                                    ...
                                                    if (
                                                      ...
                                                      if (weekReportData) {
                                                        ...
                                                        if (
                                                          'ETF_TRANSACTION_REPORT' in
                                                            weekReportData &&
                                                          'ETF_HOLDING_VALUATION' in
                                                            weekReportData &&
                                                          'ETF_HOLDING_VALUATIONPrevious' in
                                                            weekReportData
                                                        ) {
                                                          ...
                                                          if (
                                                            ...
                                                            weekReportData[
                                                              'ETF_TRANSACTION_REPORT_Prev'
                                                            ] =
                                                              previousWeekReportData[
                                                                'ETF_TRANSACTION_REPORT'
                                                              ]
                                                            weekReportData[
                                                              'ETF_HOLDING_VALUATION_Prev'
                                                            ] =
                                                              previousWeekReportData[
                                                                'ETF_HOLDING_VALUATION'
                                                              ]
                                                            weekReportData[
                                                              'ETF_HOLDING_VALUATIONPrevious_Prev'
                                                            ] =
                                                              previousWeekReportData[
                                                                'ETF_HOLDING_VALUATIONPrevious'
                                                              ]
                                                          }
                                                          const totalLiquidBeesGains =
                                                            calcTotalETFGainsLiquidDB(
                                                              weekReportData,
                                                              clientBrokerDetails
                                                            )
                                                          const totalOtherETFGains =
                                                            calcTotalETFGainsOthersDB(
                                                              weekReportData,
                                                              clientBrokerDetails
                                                            )
                                                    ...
                                                    if (
                                                      ...
                                                      if (weekReportData) {
                                                        ...
                                                        const result =
                                                          ...
                                                          charges +
                                                          totalAdjustments
                                                        const resultsPercentage =
                                                          calculatePercentage(
                                                            result,
                                                            aum
                                                          )
                                                        allWeeksResultsPercentage[
                                                          clientCode
                                                        ].push(
                                                          resultsPercentage
                                                        )
                                                    ...
                                                    if (
                                                      ...
                                                      if (weekReportData) {
                                                        ...
                                                        if (
                                                          ...
                                                          const lastPORTFOLIO_VALUE_entryDate =
                                                            weekReportData[
                                                              'PORTFOLIO_VALUE'
                                                            ][
                                                              weekReportData[
                                      ...
                                      <div
                                        className={`card-body p-0 ${
                                          currentTab == clientCode + '_alldata'
                                            ? ''
                                            : 'd-none'
                                        }`}
                                      >
                                        <div className="card border-0">
                                          ...
                                          {weeks.length > 0 &&
                                            weeks.map((wke, indx) => {
                                              ...
                                              if (wkData) {
                                                const formatted_ledger =
                                                  'LEDGER' in wkData
                                                    ? wkData['LEDGER']
                                                    : []
                                                const formatted_fno_profits =
                                                  'FNO_PROFITS' in wkData
                                                    ? wkData['FNO_PROFITS']
                                                    : []
                                                const formatted_portfolio_value =
                                                  'PORTFOLIO_VALUE' in wkData
                                                    ? wkData['PORTFOLIO_VALUE']
                                                    : []
                                                ...
                                                return (
                                                  <div
                                                    className={`card-body border-0 p-0 ${
                                                      currentAllTradeDataTab ==
                                                      'alldatatab_' +
                                                        clientCode +
                                                        wke.formattedEnd
                                                  ...
                                                  >
                                                    ...
                                                    <FnODBTable
                                                      records={
                                                        formatted_fno_profits
                                                      }
                                                    />
                                                    <h2 className="px-4 pt-4">
                                                      Portfolio Value
                                                    </h2>
                                                    <PortfolioValueDBTable
                                                      records={
