...
import { formatETFGainsResponseData, getTradeData } from "../core/_requests";
import { ETFDBTable } from "./ETFDBTable";
...
export default function Landing(props: Props) {
    ...
    async function loadTradeData() {
        ...
        clientBrokers.map(async (clientBroker) => {
            const etfs_isins = clientBroker.broker.etfs_isins;
            const etfs_scripcodes = clientBroker.broker.etfs_scripcodes;
            const reqData = {
                "data_type": "etf_gains", //ledger, fno_profits, portfolio_value, etf_gains
                "daterange": `${startDate},${moment(startDate,"YYYY-MM-DD").subtract(8,'days').format("YYYY-MM-DD")}`,
                "client_code": clientBroker.clientCode,
                "client_id": clientBroker.boidDematNumber
            }
            // console.log(etfs_isins,etfs_scripcodes)
            if(currentBrokerTab == "angel" && clientBroker.broker.code == "angle-one-broking"){
                ...
                try{
                    ...
                    if(angelResponse.status == 201){
                        // const angelResponseData = angelResponse.data.data
                        // const finalData = formatETFGainsResponseData("angel",angelResponseData,etfs_scripcodes, etfs_isins)
            ...
            if(currentBrokerTab == "iifl" && clientBroker.broker.code == "iifl-securities"){
                ...
                try{
                    ...
                    if(iiflResponse.status == 201){
                        // const finalData = formatETFGainsResponseData("iifl",iiflResponse,etfs_scripcodes, etfs_isins)
                                        ...
                                        <div className="col-12">
                                            <div className="card">
                                                <div className="card-header">
                                                    <h3 className="card-title">Client ETF Gains</h3>
                                                ...
                                                <div className="card-body p-0">
                                                    <div className="card">
                                                        ...
                                                        {currentBrokerTab == "angel" && (
                                                            <div className="card-body p-0">
                                                                <ETFDBTable records={angelRecords} />
                                                            </div>    
                                                        )}
                                                        {currentBrokerTab == "iifl" && (
                                                            <div className="card-body p-0">
                                                                <ETFDBTable records={iiflRecords} />
                                                            </div>    
                                                        )}
                                                    </div>
                                                </div>
